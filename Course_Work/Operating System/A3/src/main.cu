...
// data input and output --- Input, output buffer
...
int main() {
    ...
    //Load all files into the input buffer.
    int input_size = load_binaryFile(DATAFILE, input, STORAGE_SIZE);
    // Call the kernel function. INVERT_PAGE_TABLE_SIZE: 16 kb
    ...
    mykernel<< <1, 1, INVERT_PAGE_TABLE_SIZE>>>(input_size); //Input size = 128kb
...
//TA will write input buffer and load it to the virtual memory.
// 327769： 32kb + 1, how many page fault you will encounter in the read part.
// What is a snapshot? Look at the command at the virtual_memory.cu.
// load input_size elements from the virtual memory to the result buffer.
// Then write the result buffer back to a output file.